...
> 课程报告 · 2026-04-20 · 配套仓库：[`agentguard/`](../)
...
本节概括 5 个开源项目 + 2 份标准规范的技术路线，每条按 **方法 → 独特优势 → 不足** 陈述。
...
**7 个测试用例矩阵**（表 3，覆盖评分表要求的 "≥5 个不同测试用例"）：
...
7. 《面向自主 AI Agent 的零信任安全网关架构：基于 Sidecar 模式防范非幂等重试与提示词注入的研究报告》，项目随附文档 [`docs/面向自主 AI Agent 的零信任安全网关架构.md`](面向自主%20AI%20Agent%20的零信任安全网关架构.md)
8. 本课程配套代码仓库 AgentGuard，23 个 pytest 结果与 6 个 demo 脚本产出为本报告定量数据来源。
